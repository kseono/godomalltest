        ...
        if (_.isUndefined(data.error)) {
            $.each(data, function (key, value) {
                ...
                if (value.mobilePopupSkin == 'top') {
                    ...
                    main_popup_layer(value, value.mobilePopupKindFl);
                }
            });
        }
    });
...
function hide_top_popup(type, code, t) {
    ...
    var head_top = Number($('#header').css('top').replace('px', '')) - $('#' + code).height();
    headTotalHeight = head_top;
    $('#header, #slide').css('top', head_top + 'px');
...
$(window).scroll(function(){
    ...
    if (headTop > $(window).scrollTop()) {
        $('#header, #slide').css('top', headTotalHeight - $(window).scrollTop() + 'px');
    } else {
        $('#header, #slide').css('top', '0px');
    }
});
