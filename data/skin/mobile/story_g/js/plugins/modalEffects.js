    ...
    $.Modal.prototype = {
        ...
        hide: function() {
            ...
            if (hideEvent.isDefaultPrevented())
                return;
            //안드로이드 3,4,4.1에서 pushState 작동 안함
//			if(_.isUndefined(window.history.pushState)) location.hash = '';
//			else window.history.pushState("", document.title, window.location.pathname);//HTML5 API
			
			var hideScrollFlag = false;
            if($('.ly_show').attr('id') == 'popup-board-view'){
                hideScrollFlag = true;
            }
            ...
            $('.st-buy').show();
            $('.st-pusher').show();
//            $('#wrap').css({height:'inherit',overflow:'inherit'});
//			$('#container').css({overflow:'hidden'});
        ...
        show: function() {
            ...
            this.$wrapElement.find('.ly_ct').height($(window).height()-73);
            ...
            // $('.st-pusher').hide(); // 모달창 오픈 후 닫을 때 컨텐츠의 스크롤 위치 유지 시키기 위해서 주석 처리
            ...
            /* 아이폰 모달 닫으면 wrap에 높이가 잡힘
             $(window).bind('resize', function(e) {
             if (self.$el.hasClass('ly_show')) {
             self.$wrapElement.height($(window).height());
             $('#wrap').css({height: $(window).height()});
             }
             }); */

            //this.$overlayClass.unbind( 'click', this.hide );
            //this.$overlayClass.bind( 'click', this.hide );
    ...
    $.fn.modal = function(option) {
        ...
        $(document).one('click', '.ly_btn_close, .ly_overlay', function(ev) {
            ev.stopPropagation();

            if ($('.ly_show').attr('id') == 'popup-board' && $('#popup-board > .ly_wrap').length > 0) {
                $('#popup-board > .ly_wrap').remove();
            }

            if (location.hash != '') {
                var currentModalId = $('.ly_show').attr('id');
                if (currentModalId != 'popup-cash-receipt-request' && currentModalId != 'popup-tax-invoice-request') {
        ...
        $('.ly_pop').each(function(idx) {
            ...
            if (target == location.hash) {
                ...
                if ($(target).hasClass('ly_show')) {					
                    ...
                    if( $(target).attr('id') == 'popup-board-view'){						
    ...
    $(document).on('focusin focusout', '.ly_show :input:not([type="button"], [type="submit"], [type="checkbox"], [type="radio"], [id="btnCouponApply"])', function(e){
        ...
        switch($('.ly_show').attr('id')) {
            ...
            case "popup-exchange":
            case "popup-board":
            case "popup-board-view":
            case "popup-board-write":
            case "popup-shipping":
            case "popup-delivery-method":
            case "popup-coupon-apply":
