...
var buyEffects = (function() {
    ...
    function init() {
        ...
        var container = $('.st-container'),
            menu = container.find('.st-buy'),
            buttons = $('.st-btn-buy, .st-buy-close'),
...
//			eventType = 'click',
        resetBuy = function() {
            container.removeClass('st-buy-open');
            ...
            bodyBuyClickFn = function(event) {
                ...
                container.find('.st-buy').css("bottom","100");

                // document click시 어디를 눌렀는지 확인하여 처리
                var target = $(event.target);
                //쿠폰적용하기 클릭시에는 옵션창 닫히지 않음
                if(target.closest('.delete-goods').length == 0 && target.closest('#popup-coupon-apply').length =='0' && $(target).closest('.js-coupon-apply-close').length ==0) {
//                    if (target.closest('.st-buy-close').length || !target.parents('.st-buy').length) {
                    if (target.closest('.st-buy-close').length) {

                        $('.goods-view-image-slider').trigger('beforeChange');
                        resetBuy();
                        $(document).off(eventType, bodyBuyClickFn);

                        $('.js-multi-option').each(function () {
                            if($(this).find("div.optbx").css("display") =='block') {
                                $(this).find(".js-option-select ").click();
                                $(this).css("position","");
                                $(this).css("z-index","");
                                $(this).css("top","");
                                $(this).css("height","");
                        ...
                        setTimeout(function() {
                            $(".js-goods-add-pay").hide();
                            $(".st-buy-content").hide();
                            $(".js-goods-view-buy-btn").show();
                            $(".st-buy-top-btn").height("90px");
                            $(".st-btn-buy-outer").removeClass("hidden");
                        }, 100);
        ...
        buttons.each(function(i) {
            ...
            $(this).on(eventType, function(ev) {
                if (!container.hasClass('st-buy-open')) {
                    $(".js-goods-add-pay").show();
                    $(".st-buy-content").show();

                    $(".js-goods-view-buy-btn").hide();
                    $(".st-buy-top-btn").height("16px");
                    ...
                    setTimeout(function() {
                        //menu.css("bottom","0");
                        container.addClass('st-buy-open');
                    }, 25);
