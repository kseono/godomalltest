...
var gd_goods_view = function () {
    ...
    this.option_price_display = function(inputName)
    {
        ...
        else
        {
            ...
            if (setOptionPriceFl == 'y' && optionPrice) {
                ...
                $('[id^=option_display_item_'+displayOptionkey+'] span').eq(0).append(optionDisplayTextPrice);
            }

            $(frmId + ' div#option_display_item_' + displayOptionkey+' div.optionKey_'+arrTmp[0] + '  .goods-cnt').on('click', function(e){
    ...
    this.option_select = function(inputName,thisKey,stockViewFl)
    {
        ...
        // multiOptionClsNm : 상품상세 관련상품은 js_multi_option 대입, 장바구니/찜리스트/분류리스트(장바구니형) 옵션변경은 js-multi-option 대입
        var multiOptionClsNm = ($(frmId+' li.js_multi_option'+setOptNo).length > 0 ? 'js_multi_option' : 'js-multi-option');
        ...
        for (var i = 0; i <= thisKey; i++) {
            optionVal[i]	= $(frmId+' li.'+multiOptionClsNm+setOptNo+'.optionNo_'+i+' ul.optlst li.selected').data('params');
            // 선택값이 없는경우 disabled 처리
            if (optionVal[i] == '') {
                for (var j = (i+1); j <= optionCnt; j++) {
                    if (j != 0) {
                        $(frmId+' li.optionNo_'+j+' .js-option-select'+setOptNo).addClass('disabled');
                    }
                }
        ...
        $.post('../goods/goods_ps.php',{'mode' : 'option_select', 'optionVal' : optionVal, 'optionKey' : thisKey, 'goodsNo' : setGoodsNo, 'mileageFl' :setMileageFl}, function(data){
            ...
            for (var i = 0; i <= optionCnt; i++) {
                if (i <= data.nextKey) {
                    ...
                    if (i == data.nextKey) {
                        $(frmId+' li.'+multiOptionClsNm+setOptNo+'.optionNo_'+i+' ul.optlst').html("");
                        ...
                        $(frmId+' li.'+multiOptionClsNm+setOptNo+'.optionNo_'+i+' ul.optlst').html(addSelectOption);
                    }
                } else {
                    if (i != 0) {
                        $(frmId+' li.optionNo_'+i+' .js-option-select'+setOptNo).addClass('disabled');
                    }
                }
            }
        ...
        if(displayOptionDisplay)
        {
            ...
            if($(frmId+' #option_text_display_' + displayOptionkey).html() !== '' && noChangeFl == 'n') {
                ...
                if(optionTextInputCnt <= emCnt) {
                    ...
                    var optionName = $lastDiv.find('span').first().text();
