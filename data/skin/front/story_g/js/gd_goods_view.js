...
var gd_goods_view = function () {
    ...
    this.count_change = function(inputName,goodsFl)
    {
        ...
        $('button.goods-cnt').attr('disabled', true);
        $('button.add-goods-cnt').attr('disabled', true);
        var minCnt =parseInt(setMinOrderCnt);
        var maxCnt = parseInt(setMaxOrderCnt);
    ...
    this.option_price_display = function(inputName)
    {
        ...
        if(setOptNo != "" &&  setOptNo != undefined) {
            if ($(frmId + ' select[name="optionSnoInput' + setOptNo + '"]').val() == '0'){
                ...
                $('#option_display_item_'+setOptNo+' > div').remove();
                return;
            }
        }
        ...
        if(arrTmp[3]) $(frmId + ' input[name="set_goods_stock'+setOptNo+'"]').val(parseFloat(arrTmp[3].trim()));
        ...
        if($(frmId+" div.optionKey"+setOptNo+"_"+arrTmp[0]).length && (setOptNo == "" ||  setOptNo == undefined) && setOptionTextFl !='y') {
        ...
        else
        {
            ...
            if (setOptionPriceFl == 'y' && optionPrice) {
                ...
                $('[id^=option_display_item_'+displayOptionkey+'] .name > strong').append(optionDisplayTextPrice);
            }

            $(frmId + ' div#option_display_item_' + displayOptionkey+' div.optionKey'+setOptNo+'_'+arrTmp[0] + '  .goods-cnt').on('click', function(e){
                setControllerName.count_change(this,1);
            });

            $(frmId+' div.optionKey'+setOptNo+'_'+arrTmp[0] + '  button.delete-goods').on('click', function(e){
            ...
            if(setCartTabFl =='y') {
                ...
                if (setOptionPriceFl == 'y' && optionPrice) {
                    ...
                    var optionDisplayTextPrice = ' ('+ addPlus + gdCurrencySymbol + gd_money_format(optionPrice) + gdCurrencyString+')';
                    $('#frmCartTabViewLayer div.optionKey'+setOptNo+'_' + arrTmp[0] + ' .name > strong').append(optionDisplayTextPrice);
                }

                $('#frmCartTabViewLayer div.optionKey'+setOptNo+'_'+arrTmp[0] + '  .goods-cnt').on('click', function(e){
                    var datakey = $(this).val().split(setStrDivision);
                    $('#option_display_item_'+datakey[1]).find('button[class="'+$(this).attr('class')+'"]').click();
                });

                $('#frmCartTabViewLayer div.optionKey'+setOptNo+'_'+arrTmp[0] + '  button.delete-goods').on('click', function(e){
    ...
    this.option_select = function(inputName,thisKey, nextVal,stockViewFl)
    {
        ...
        $.post('../goods/goods_ps.php',{'mode' : 'option_select', 'optionVal' : optionVal, 'optionKey' : thisKey, 'goodsNo' : setGoodsNo, 'mileageFl' :setMileageFl}, function(data){
            if (typeof data.optionSno == 'string') {
                ...
                else setControllerName.option_price_display($(frmId+' input[name=\'optionSnoInput'+setOptNo+'\']'));

                return true;
            } else {
                if(setOptNo != "" &&  setOptNo != undefined) {
                    $('select[name="optionSnoInputCnt' + setOptNo + '"]').prop('disabled', true);
                    ...
                    if (i == data.nextKey) {
                        ...
                        for (var j = 0; j < data.nextOption.length; j++) {
                            if(data.optionViewFl[j] =='y') {
                                ...
                                if (data.optionDivisionDisabledMark == 't' && ((stockFl == 'y' && data.stockCnt[j] <  setOriginalMinOrderCnt)  || (stockFl == 'y' && setFixedOrderCnt == 'option' && data.stockCnt[j] < setMinOrderCnt) || (stockFl == 'y' && data.stockCnt[j] == 0)  || data.optionSellFl[j] == 'n' || data.optionSellFl[j] == 't')) {
                                    addSelectOption	+= ' disabled="disabled"';
                                }
                                // 마지막 옵션의 경우 재고 체크 (품절이면 disabled 처리) : 구버전 레거시 조건
                                else if ((parseInt(data.nextKey) + 1) == parseInt(optionCnt) && ((stockFl == 'y' && data.stockCnt[j] <  setOriginalMinOrderCnt)  || (stockFl == 'y' && setFixedOrderCnt == 'option' && data.stockCnt[j] < setMinOrderCnt) || (stockFl == 'y' && data.stockCnt[j] == 0)  || data.optionSellFl[j] == 'n' || data.optionSellFl[j] == 't')) {
                                    addSelectOption	+= ' disabled="disabled"';
                                }
    ...
    this.option_text_select = function(inputName) {
        ...
        if(displayOptionDisplay)
        {
            ...
            //if(setOptionFl =='y') {
                var optionItemNo = $(frmId+' div[id*=\'option_display_item_'+ setOptNo +'\']').length-1;
            //} else {
            //    var optionItemNo = 0;
            //}
                ...
                if(optionTextInputCnt < emCnt) {
                    ...
                    if(setCartTabFl =='y') {
                        ...
                        if (setOptionPriceFl == 'y' && optionPrice) {
                            ...
                            var optionDisplayTextPrice = ' ('+ addPlus + gdCurrencySymbol + gd_money_format(optionPrice) + gdCurrencyString+')';
                            $('#frmCartTabViewLayer div.optionKey'+setOptNo+'_' + optionSno + ' .name > strong').append(optionDisplayTextPrice);
                        }

                        $('#frmCartTabViewLayer div.optionKey'+setOptNo+'_'+optionSno + '  .goods-cnt').on('click', function(e){
                            var datakey = $(this).val().split(setStrDivision);
                            $('#option_display_item_'+datakey[1]).find('button[class="'+$(this).attr('class')+'"]').click();
                        });

                        $('#frmCartTabViewLayer div.optionKey'+setOptNo+'_'+optionSno + '  button.delete-goods').on('click', function(e){
                            $('#frmView #'+$(this).data('key')+'  button.delete-goods').click();
                        });
            ...
            $(frmId+' input[name*=\'optionTextInput'+setOptNo+'\']').each(function (key) {
                if ($(this).val()) {
                    ...
                    var optionTextNm = $(frmId+' span.optionTextNm'+ setOptNo +'_'+key).text();
            ...
            if ($(frmId + ' tr.optionKey'+setOptNo+'_' + tmpStr[0] + optionTextKey).length) {
                ...
                if(setCartTabFl =='y') {
                    ...
                    $('#frmCartTabViewLayer .'+displayOptionDisplay+' div.'+checkKey).attr('class', 'this z ' + tmpStr[0] + optionTextKey);
                }
    ...
    this.goods_calculate = function(frmId,goodsFl,itemNo,goodsCnt) {
        ...
        if(goodsFl)
        {
            ...
            $(frmId+' .option_price_display_' + itemNo).html( gd_money_format(((optionTotalPrice+goodsPrice)*goodsCnt).toFixed(setDecimal)));
