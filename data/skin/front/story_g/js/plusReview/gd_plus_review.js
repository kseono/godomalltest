$(function () {
    ...
    $(document).on('click', '.layer-close, .close, .js-select-confirm', function(e) {
        var plusReviewPopup = $('#plusReviewPopup.sys-pop');
        var layerDim = $('#layerDim');
        if ($(this).hasClass('js-select-confirm')) {
            if ($('#frmSelect input[type=radio][name="goodsNo[]"]:checked').length < 1) {
                return false;
            }
        }
        if ($('.goods-select-layer').parents('.layer-wrap').hasClass('dn') && $('#plusReviewPopup').length > 0) {
