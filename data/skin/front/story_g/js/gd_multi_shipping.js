...
$(function(){
    $('input[name="multiShippingFl"]').click(function(){
        ...
        if (checked === true) {
            $('.shipping-info table:eq(0) th.ta-l:eq(0)').html(__('메인 배송지'));
            $('.shipping-info table:eq(0) .orderVisitTr th:eq(0)').html(__('메인 방문수령 정보'));
            $('#memberinfoApplyTr2').find('input[name="reflectApplyMember"]').prop('checked', false);
            $('#memberinfoApplyTr2').find('label').removeClass('on');
            $('.shipping-add-btn, .select-goods-tr, .shipping-visit-add-btn').removeClass('dn');
            $('.shipping-add-btn').trigger('click');
        } else {
            $('.shipping-info table:eq(0) th.ta-l:eq(0)').html(__('배송지 확인'));
            $('.shipping-info table:eq(0) .orderVisitTr th:eq(0)').html(__('방문수령 정보'));
            $('#memberinfoApplyTr').show();
            $('.select-goods').addClass('dn').find('table').empty();
            $('input[name^="selectGoods"]').val('');
            $('input[name^="multiDelivery"]').val(0);
            $('.shipping-add-btn, .select-goods-tr, .shipping-visit-add-btn').addClass('dn');
    ...
    $('.shipping-add-btn, .shipping-visit-add-btn').click(function(){
    ...
    $(document).on('click', '.shipping-remove-btn, .shipping-visit-remove-btn', function(){
        ...
        if ($('.shipping-info-table').length <= 1) {
            ...
            for (var i = $('.shipping-info-table').length; i > index; i--) {
                $('.shipping-info-table:eq(' + (i - 1) + ') th:eq(0) span.no').html(i - 1);
    ...
    $(document).on('click', '.postcode-search', function(){
        ...
        postcode_search('receiverZonecodeAdd[' + no + ']', 'receiverAddressAdd[' + no + ']', 'receiverZipcodeAdd[' + no + ']');
    });

    $(document).on('click', 'div.del button', function(){
        var $target = $(this);
        ...
        switch (type) {
            case 'goods':
                var addgoodsCnt = $target.closest('table').find('.del button[data-type="addGoods"][data-cart-sno="' + cartSno + '"]').length;
        ...
        if (totalGoodsCnt > 0) {
            $.ajax({
                ...
                $target.closest('.select-goods-tr').find('input[name^="multiDelivery"]').val(getData.deliveryCharge);
                $target.closest('.select-goods-tr').find('input[name^="selectGoods"]').val(data);
                if ($('.delete-add-goods[data-parent-cart-sno="' + parentCartSno + '"]').length > 1) {
                    var moveInfo = $('.delete-add-goods[data-parent-cart-sno="' + parentCartSno + '"]');
                    var deliveryInfo = $target.closest('table').find('.delivery-info[data-parent-cart-sno="' + parentCartSno + '"]');
                    var index = $('.delete-add-goods[data-parent-cart-sno="' + parentCartSno + '"]').index($target);
                    var rowspan = deliveryInfo.attr('rowspan');
                    if (type == 'goods' && index == 0) {
                        ...
                        $('.delete-add-goods[data-parent-cart-sno="' + parentCartSno + '"]:eq(1)').closest('td').before(deliveryInfo);
                    } else {
    ...
    $(document).on('click', '.shipping-goods-select', function (e) {
        ...
        $.ajax({
            ...
            $('#cartSelectGoods').html('').empty().append(data).find('>div').center();
        }).error(function (e) {
            if(e.responseText) {
