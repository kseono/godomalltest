...
var gd_member2 = (function ($, _, window, document, undefined) {
    ...
    validate_settings = {
        defaults: {
            ...
            errorPlacement: function (error, element) {
                try {
                    ...
                    if ($error.length === 0) {
                        ...
                        if (closest.find('div.c-red').length == 0) {
            ...
            highlight: function (element) {
                ...
                $(element).closest('div').css('border', '4px solid #ff4c2e');
            },
            unhighlight: function (element) {
                if ($(element).prop('type') != 'checkbox' && $(element).prop('type') != 'radio') {
                    $(element).closest('div').css('border', '1px solid #d0d0d0');
                }
            },
            onkeyup: onkeyup,
    ...
    function ajax_validate(element, data) {
        ...
        $ajax.done(function (data, textStatus, jqXHR) {
            ...
            var blue = closest.find('.c-blue');
            if (blue.length < 1) {
                blue = $('<div class="c-blue"></div>');
                closest.find('.c-red, .c-blue').remove();
                closest.append(blue);
            }
            if (_.isUndefined(code) && _.isUndefined(message)) {
                blue.text(data);
            } else {
                blue.addClass('c-red').text(message);
                blue.removeClass('c-blue');
            }
        });
    }
    ...
    replace_keyup = {
        ...
        gdEngKorNum: function (element) {
            // IE11에서 초중종성 분리되는 현상때문에 test 후 replace 처리로 변경
            if (regexp_test(element.value, /[^a-zA-Zㄱ-ㅎㅏ-ㅣ가-힣0-9\u119E\u11A\u318D\u2022\u2025a\u00B7\uFE552]/)) {
    ...
    function validation_submit(func) {
        ...
        if (validate.form()) {
            ...
            go_to_by_scroll($('div.c-red:first', form));
        }
    }
    ...
    function display_validation($target) {
        ...
        var $closest = $target.closest('td');
        if ($closest.length === 0) {
            $closest = $target.closest('div.input-pw');
        }
        $closest.find('div.txt-field').removeClass('error');
        $closest.find('div.msg').remove();
        $closest.find('div:first').after('<div class="msg c-blue">' + $target.data('valid-message') + '</div>');
    }
    ...
    function display_invalidation($target) {
        ...
        $closest.find('div:first').after('<div class="msg c-red error">' + $target.data('valid-message') + '</div>');
    }
