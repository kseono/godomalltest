...
function gd_member_event_fixed_popup(data)
{
    ...
    popupHtml +=
        ...
        '    <div id="memberEventPopup_'+ data.eventType +'_form" class="sys-pop">' +
        '        <div class="box">' +
        '            <div class="view">';
    ...
    if (data.popupContentType === 'direct') {
        ...
        var popupContentImage = (data.eventType === 'modify') ? '/admin/gd_share/img/member/member_modify_event_' + data.domainFl + '_02.png' : '/admin/gd_share/img/member/member_life_event_' + data.domainFl + '_02.png';
        var eventBtnName = (data.eventType === 'modify') ? '회원정보 수정하기' : '평생회원 전환하기';
        popupHtml +=
            '                <p><img src="'+ popupContentImage +'" class="js-smart-img"></p>';
    ...
    if (data.todayUnSeeFl === 'y') {
        popupHtml +=
            ...
            '                    <input type="checkbox" id="todayUnSee_memberEventPopup_'+ data.eventType +'" class="checkbox" onclick="popup_cookie(\'memberEventPopup_'+ data.eventType +'\', this);">' +
            '                </span>' +
            '            </div>';
            <!-- 오늘 하루 보이지 않음 : end -->
    }
